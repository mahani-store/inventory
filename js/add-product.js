...
async function addProduct() {
  ...
  document
    ...
    .addEventListener("submit", async function (e) {
      ...
      const hargaStr = document.getElementById("harga")?.value.trim() || "0";
      const stokStr = document.getElementById("stok")?.value.trim() || "0";
      const deskripsi =
...
document
  ...
  .addEventListener("input", updatePreviewHPP);
document.getElementById("stok").addEventListener("input", updatePreviewHPP);

function updatePreviewHPP() {
  const total = Number(document.getElementById("hargaPokokTotal").value) || 0;
  const perUnit =
    Number(document.getElementById("hargaPokokPerUnit").value) || 0;
  ...
  if (perUnit > 0) {
    ...
    valueEl.textContent = hpp.toLocaleString("id-ID") + " (dari total / stok)";
    previewEl.classList.remove("hidden");
  } else {
